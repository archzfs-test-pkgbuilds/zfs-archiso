...
pkgver=0.7.5_4.13.12.1
pkgrel=1
makedepends=("linux-headers=4.13.12" "spl-archiso-linux-headers")
arch=("x86_64")
url="http://zfsonlinux.org/"
source=("https://github.com/zfsonlinux/zfs/releases/download/zfs-0.7.5/zfs-0.7.5.tar.gz")
sha256sums=("SKIP")
license=("CDDL")
depends=("kmod" "spl-archiso-linux" "zfs-utils-common=0.7.5" "linux=4.13.12")
...
package_zfs-archiso-linux-headers() {
    ...
    conflicts=( 'zfs-archiso-linux-git-headers' 'zfs-linux-hardened-headers' 'zfs-linux-hardened-git-headers' 'zfs-linux-lts-headers' 'zfs-linux-lts-git-headers' 'zfs-linux-headers' 'zfs-linux-git-headers' 'zfs-linux-vfio-headers' 'zfs-linux-vfio-git-headers' 'zfs-linux-zen-headers' 'zfs-linux-zen-git-headers' )
    cd "${srcdir}/zfs-0.7.5"
    make DESTDIR="${pkgdir}" install
    rm -r "${pkgdir}/lib"
